...
if [ ! -f "workspace/prd.json" ]; then
    echo "Erro: workspace/prd.json não encontrado."
...
echo "[AGENT-INSTRUCTION] Leia a seção 'meta' do prd.json para saber a stack e os comandos de verificação."
echo ""
echo "[AGENT-INSTRUCTION] PASSO 0b — CARREGAR MEMÓRIA (OBRIGATÓRIO):"
echo "[AGENT-INSTRUCTION] 1. Leia workspace/memory/agent-brain.md (memória cross-project)"
...
echo "[AGENT-INSTRUCTION] 4. Leia workspace/memory/[projeto].md (se existir)"
echo ""
echo "[AGENT-INSTRUCTION] LOOP POR TAREFA (repita até não haver mais tarefas pendentes):"
echo "[AGENT-INSTRUCTION] --- EXECUTE ---"
echo "[AGENT-INSTRUCTION] 1. Leia workspace/prd.json e pegue a primeira tarefa com status 'pending'"
echo "[AGENT-INSTRUCTION] 2. Atualize status para 'in_progress'"
echo "[AGENT-INSTRUCTION] 3. Leia o campo 'instructions' da tarefa (auto-suficiente)"
...
echo "[AGENT-INSTRUCTION] 7. Se testes E2E existirem: yarn playwright test"
...
echo "[AGENT-INSTRUCTION] 10. Há algo a aprender? Documente em workspace/memory/[projeto].md"
echo "[AGENT-INSTRUCTION]     Se for padrão recorrente: atualize workspace/memory/agent-brain.md"
echo "[AGENT-INSTRUCTION] → Volte ao passo 1 com a próxima tarefa"
echo ""
echo "[AGENT-INSTRUCTION] RELATÓRIO A CADA TAREFA:"
echo "[AGENT-INSTRUCTION] Informe: '✓ Task [id]/[total]: [nome] — [N] restantes'"
echo ""
